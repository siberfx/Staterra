...
function RelatedBlog({ currentSlug, limit = 3, postRoute }) {
    ...
    <section className="py-20 bg-gray-50 border-t border-gray-100">
      <div className="container-page">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {items.map((item) => {
            const title = typeof item.title === 'object' ? item.title?.nl ?? item.title ?? '' : (item.title ?? '')
            const date = typeof item.date === 'object' ? item.date?.nl ?? item.date ?? '' : (item.date ?? '')
            const category = typeof item.category === 'object' ? item.category?.name : (item.category ?? null)
            return (
              <Link
                key={item.slug}
                to={postRoute ? postRoute(item.slug) : R.blogPost(item.slug)}
                ...
                <div className="p-8">
                  <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
                    {(item.author_avatar ?? item.author?.avatar) && (
                      <img src={item.author_avatar ?? item.author?.avatar} alt="" className="w-5 h-5 rounded-full object-cover flex-shrink-0" />
                    )}
                    {category && (
