...
const DEFAULT_HERO = {
  badge: "",
  title: "Volledige regie over uw digitale systemen",
  subtitle:
    "Alles wat wij bouwen is open source. U bezit de code, u bezit de data en u bent niet afhankelijk van één leverancier. Dit is geen optie, maar de basis van hoe wij werken. Daarnaast implementeren, integreren en beheren we bestaande open source oplossingen — zoals documentplatformen (bijv. Nextcloud), cloud-infrastructuur (OpenStack) en sectorgerichte toepassingen (zoals VLAM). U behoudt regie, wij zorgen dat het werkt en blijft werken.",
  cta_primary_text: "Verkenning starten",
  cta_primary_url: null,
  cta_secondary_text: null,
  cta_secondary_url: null,
};

function OpenSourceHeroSplit({ content, imageSrc }) {
  const {
    badge,
    title,
    subtitle,
...
/**
 * Open-source pagina hero: zelfde patroon als Woo/Samen (tekst + beeld, witte achtergrond).
 * Props: `hero` uit page-sections `hero`, `apiOnly` voor CMS-modus.
 */
export function OpenSourceHero({ hero: c, apiOnly = false } = {}) {
  ...
  return (
    ...
    >
      <div className="relative w-full">
        <OpenSourceHeroSplit content={content} imageSrc={imageSrc} />
      </div>
    </section>
  );
}
