...
/** CMS’de `image_url` yoksa: her mount’ta Picsum’dan farklı bir foto (önbelleği kırmak için benzersiz sorgu). */
...
const DEFAULT_HERO = {
  badge: "Oplossing",
  title: "Werkende Woo-oplossing — direct inzetbaar",
  subtitle:
    "Actieve openbaarmaking en documentbeheer, zonder lange trajecten — bewezen en direct inzetbaar binnen de overheid.",
...
function WooHeroSplit({ content, imageSrc }) {
  const {
    badge,
    title,
    subtitle,
...
export function Header62({ hero: c, apiOnly = false } = {}) {
  ...
  return (
    ...
    >
      <div className="relative w-full">
        <WooHeroSplit content={content} imageSrc={imageSrc} />
      </div>
    </section>
  );
}
